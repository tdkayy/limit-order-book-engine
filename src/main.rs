...
async fn place_order(
    ...
    // Route the incoming order through the matching engine.
    // execute_order() matches against resting orders and only
    // adds the unfilled remainder to the book.
    let fills = {
    ...
    Json(OrderResponse {
        ...
        message: if fills.is_empty() {
            "Order placed successfully".into()
        } else {
            "Order processed successfully".into()
        },
